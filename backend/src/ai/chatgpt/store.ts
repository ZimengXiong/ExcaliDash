...
export const savePendingAuth = async (
  ...
  // Keep one pending flow per user. Starting again invalidates the earlier
  // state and prevents abandoned connect attempts from accumulating.
...
const refreshInFlight = new Map<string, Promise<EnsureFreshResult>>();
 ...
 */
const ensureFreshAuthInternal = async (
  prisma: PrismaClient,
  userId: string,
...
export const ensureFreshAuth = async (
  ...
  const pending = ensureFreshAuthInternal(prisma, userId);
  refreshInFlight.set(userId, pending);
  try {
    return await pending;
  } finally {
    if (refreshInFlight.get(userId) === pending) {
