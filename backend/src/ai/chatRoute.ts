...
const parseCanvasImage = (value: unknown): string | undefined => {
  if (typeof value !== "string" || value.length > MAX_CANVAS_IMAGE_CHARS) return undefined;
...
const adapterFor = (provider: string): AiProviderAdapter | null => {
  ...
  if (provider === "openai" || provider === "gemini" || provider === "custom") return openaiAdapter;
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const clientRequestId =
        typeof body.clientRequestId === "string" && body.clientRequestId.length <= 200
      ...
      if (!drawingId || !userText || userText.length > 50_000) {
        return res
          .status(400)
          .json({ error: "Bad request", message: "drawingId and message are required" });
      }
      ...
      if (
        requestedProviderId &&
        !registry.providers.some((profile) => profile.id === requestedProviderId)
      ) {
      ...
      if (settings.provider === "chatgpt") {
        ...
        const selected = models.find((model) => model.id === requestedModel) ?? models[0];
        if (!selected) {
          return res.status(503).json({ error: "AI unavailable", message: "No supported ChatGPT models" });
        }
        if (requestedModel && !models.some((model) => model.id === requestedModel)) {
          return res.status(400).json({ error: "Bad request", message: "Unsupported ChatGPT model" });
        }
        if (
          requestedReasoningEffort &&
          !selected.reasoningEfforts.includes(requestedReasoningEffort)
        ) {
          return res.status(400).json({ error: "Bad request", message: "Unsupported reasoning effort" });
        }
        settings = { ...settings, model: selected.id };
      } else {
        const selected = settings.models.find((model) => model.id === requestedModel) ??
          settings.models[0];
        if (requestedModel && !settings.models.some((model) => model.id === requestedModel)) {
          return res.status(400).json({
            error: "Bad request",
        ...
        if (
          requestedReasoningEffort &&
          (!selected || !selected.reasoningEfforts.includes(requestedReasoningEffort))
        ) {
          return res.status(400).json({
            error: "Bad request",
