...
(pattern) => pattern.test(value) ); const isTooLarge = value.length > MAX_DATAURL_SIZE; if (isTooLarge) { throw new Error(`Image ${fileId} exceeds the ${MAX_DATAURL_SIZE} byte storage limit`); } if (hasSuspiciousContent) { file[key] = ""; } else { file[key] = value; } } else if (/^https?:\/\//i.test(value)) { const hasSuspiciousContent = suspiciousPatterns.some(
