...
isDeleted: z.boolean().optional().nullable(), opacity: z.number().optional().nullable(), link: z.string().optional().nullable(), locked: z.boolean().optional().nullable(), text: z.string().optional().nullable(), name: z.string().max(5000).optional().nullable(), fontSize: z.number().optional().nullable(), fontFamily: z.union([z.number(), z.string().max(200)]).optional().nullable(), textAlign: z.string().optional().nullable(), verticalAlign: z.string().optional().nullable(), customData: z.record(z.string(), z.any()).optional().nullable(), })
.passthrough()
.transform((element) => { const sanitized = { ...element }; if (typeof sanitized.text === "string") { sanitized.text = sanitizeElementText(sanitized.text, 5000); } if (typeof sanitized.name === "string") { sanitized.name = sanitizeElementText(sanitized.name, 5000); } if (typeof sanitized.link === "string") { sanitized.link = sanitizeUrl(sanitized.link); } return sanitized; }); export const appStateSchema = z
