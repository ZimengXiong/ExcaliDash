...
describe("Security Sanitization - Image Data URLs", () => {
  ...
  describe("configurable size limits", () => {
    ...
    it("should reject (413, not blank) a dataURL exceeding the configured limit", () => {
      ...
      expect(thrown).toBeInstanceOf(DrawingSanitizationError);
      expect((thrown as DrawingSanitizationError).statusCode).toBe(413);
      expect((thrown as DrawingSanitizationError).fileId).toBe("file-1");
    });
  ...
  describe("sanitizeDrawingData - files handling", () => {
    ...
    it("should handle multiple images with large data URLs", () => {
      ...
      const comparison = compareFilesObjects(
        files,
        result.files as Record<string, any>,
      );
      expect(comparison.isEqual).toBe(true);
      expect(comparison.differences).toHaveLength(0);
    });
    it("should reject (400) a dataURL whose base64 body contains script tags", () => {
      ...
      expect(thrown).toBeInstanceOf(DrawingSanitizationError);
      expect((thrown as DrawingSanitizationError).statusCode).toBe(400);
      expect((thrown as DrawingSanitizationError).fileId).toBe("file-1");
    });
    it("should reject (400) a javascript: protocol dataURL", () => {
      const maliciousFiles = {
        "file-1": {
          id: "file-1",
          mimeType: "image/png",
  ...
  describe("sanitizeDrawingData - preview svg handling", () => {
    it("should preserve safe SVG layout attributes needed for thumbnail rendering", () => {
      ...
      expect(result.preview).toContain(
        'viewBox="0 0 728.39453125 606.908203125"',
      );
  ...
  describe("validateImportedDrawing - with files", () => {
    ...
    it("should reject drawing with malicious content in files", () => {
      const drawing = {
        ...
        appState: { viewBackgroundColor: "#ffffff" },
        files: {
          "file-1": { id: "file-1", dataURL: "javascript:alert('xss')" },
        },
      };
      const isValid = validateImportedDrawing(drawing);
...
describe("Drawing API - Database Round-Trip", () => {
  ...
  it("should preserve image data URLs through create and read cycle", async () => {
    ...
    const retrievedDataUrl = Object.values(
      parsedFiles as Record<string, any>,
    )[0]?.dataURL;
  ...
  it("should handle multiple images with varying sizes", async () => {
    ...
    const parsedFiles = JSON.parse(retrieved!.files || "{}") as Record<
      string,
      any
    >;
    expect(parsedFiles["small-image"].dataURL).toBe(
      files["small-image"].dataURL,
    );
    expect(parsedFiles["large-image"].dataURL).toBe(
      files["large-image"].dataURL,
    );
    expect(parsedFiles["large-image"].dataURL.length).toBe(
      files["large-image"].dataURL.length,
    );
  });
...
describe("Issue #17 Regression Test - Images Not Loading Fully", () => {
  it("should reproduce and verify fix for truncated image data", () => {
    ...
    console.log(
      `Testing with image data URL of length: ${largeImageDataUrl.length}`,
    );
    const filesObject = {
      "user-uploaded-image": {
    ...
    console.log(
      "✓ Issue #17 regression test passed - image data preserved correctly",
    );
  });
  it("should handle edge case: exactly 10000 character data URL", () => {
    ...
    const result = sanitizeDrawingData({
      ...
      appState: {},
      files: {
        "boundary-test": { id: "boundary-test", dataURL: exactDataUrl },
      },
    });
    const resultFiles = result.files as Record<string, any>;
  ...
  it("should handle edge case: 10001 character data URL (just over limit)", () => {
    ...
    const result = sanitizeDrawingData({
      ...
      appState: {},
      files: {
        "over-limit-test": { id: "over-limit-test", dataURL: justOverDataUrl },
      },
    });
    const resultFiles = result.files as Record<string, any>;
