...
import { authModeEnablesAuth } from "../config";
...
export const registerCoreRoutes = (deps: RegisterCoreRoutesDeps) => {
  ...
  const getUserTrashCollectionId = (userId: string): string =>
    `trash:${userId}`;
  router.post(
    "/register",
    loginAttemptRateLimiter,
        ...
        if (isBootstrapFlow) {
          const setupCodeVerification = await verifyBootstrapSetupCode({
            ...
            maxAttempts: Math.max(
              1,
              Math.floor(config.bootstrapSetupCodeMaxAttempts),
            ),
          });
          if (!setupCodeVerification.ok) {
            const reason = (
              setupCodeVerification as { ok: false; reason: string }
            ).reason;
            if (
              reason === "unavailable" ||
              reason === "expired" ||
            ...
            if (reason === "missing") {
              return res.status(400).json({
                error: "Bad request",
                message:
                  "Bootstrap setup code is required for first admin registration",
              });
            }
            if (reason === "invalid") {
          ...
          if (username) {
            ...
            if (
              ...
              existingUsernameUser.id !== bootstrapUserId
            ) {
              return res.status(409).json({
                error: "Conflict",
          ...
          await prisma.collection.upsert({
            where: { id: trashCollectionId },
            update: {},
            create: { id: trashCollectionId, name: "Trash", userId: user.id },
          });
          ...
          try {
            ...
            if (isMissingRefreshTokenTableError(error)) {
              ...
              return res.status(503).json({
                ...
                message:
                  "Refresh token storage is unavailable. Please run database migrations.",
              });
            }
            throw error;
          }
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (bootstrapUser && bootstrapUser.isActive === false) {
          ...
          if (activeUsers === 0) {
            return res.status(409).json({
              ...
              message:
                "Initial admin account has not been configured yet. Register to bootstrap.",
            });
          }
        }
        ...
        res
          .status(500)
          .json({ error: "Internal server error", message: "Failed to login" });
      }
    },
  );
  router.post("/refresh", async (req: Request, res: Response) => {
    try {
      ...
      if (!oldRefreshToken || typeof oldRefreshToken !== "string") {
        return res
          .status(401)
          .json({ error: "Unauthorized", message: "Refresh token required" });
      }
      try {
          ...
          try {
            ...
            await prisma.$transaction(async (tx) => {
              const storedToken = await tx.refreshToken.findFirst({
                where: {
                  OR: getTokenLookupCandidates(oldRefreshToken).map(
  ...
  router.post("/logout", optionalAuth, async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Internal server error", message: "Failed to logout" });
    }
  });
  router.post(
    "/stop-impersonation",
      ...
      try {
        ...
        if (
          ...
          return res.status(403).json({
            error: "Forbidden",
            message:
              "Impersonator account is unavailable or no longer authorized",
          });
        }
        const { accessToken, refreshToken } = generateTokens(
          impersonator.id,
