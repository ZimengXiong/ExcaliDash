...
export const registerLegacySqliteImportRoutes = (
  deps: RegisterImportExportDeps,
) => {
  const {
    app,
    prisma,
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if (error instanceof ImportValidationError) {
          return res
            .status(error.status)
            .json({ error: "Invalid upload", message: error.message });
        }
        throw error;
      }
      try {
        ...
        try {
          ...
          const collectionTable = findSqliteTable(tables, [
            "Collection",
            "collections",
          ]);
          if (!drawingTable) {
            return res.status(400).json({
              ...
              message: "Missing Drawing table",
            });
          }
          const drawingsCount = Number(
            db.prepare(`SELECT COUNT(1) as c FROM "${drawingTable}"`).get()
              ?.c ?? 0,
          );
          const collectionsCount = collectionTable
            ? Number(
                db
                  .prepare(`SELECT COUNT(1) as c FROM "${collectionTable}"`)
                  .get()?.c ?? 0,
              )
            : 0;
          ...
          const migrationsTable = findSqliteTable(tables, [
            "_prisma_migrations",
          ]);
          if (migrationsTable) {
            try {
          ...
          return res.json({
            ...
            currentLatestMigration:
              await getCurrentLatestPrismaMigrationName(backendRoot),
          });
        } catch {
      ...
      try {
        ...
        try {
          ...
          const importedCollections: any[] = collectionTable
            ...
            : [];
          const importedDrawings: any[] = legacyDb
            .prepare(`SELECT * FROM "${drawingTable}"`)
            .all();
          ...
          for (
            let start = 0;
            start < preparedDrawings.length;
            start += S3_UPLOAD_CONCURRENCY
          ) {
            const batch = preparedDrawings
              ...
              .map((d, offset) => {
                ...
                return internDrawingFiles(
                  files,
                  req.user!.id,
                  finalId,
                  prisma,
                ).then((processed) => ({ i, processed }));
              });
          ...
          const result = await prisma.$transaction(async (tx) => {
            ...
            const hasTrash = importedDrawings.some(
              (d) => String(d.collectionId || "") === "trash",
            );
            ...
            for (const c of importedCollections) {
              ...
              if (!importedId) {
                ...
                await tx.collection.create({
                  data: {
                    id: newId,
                    name: sanitizeText(name, 100) || "Collection",
                    userId: req.user!.id,
                  },
                });
            ...
            const resolveImportedCollectionId = (
              ...
              const name =
                typeof rawCollectionName === "string"
                  ? rawCollectionName
                  : null;
              if (id === "trash" || name === "Trash") return trashCollectionId;
              if (id && collectionIdMap.has(id))
                return collectionIdMap.get(id)!;
              if (name && collectionIdMap.has(`__name:${name}`))
                return collectionIdMap.get(`__name:${name}`)!;
              return null;
            };
            for (let i = 0; i < preparedDrawings.length; i++) {
              const d = preparedDrawings[i];
              ...
              const existing = d.importedId
                ? await tx.drawing.findUnique({ where: { id: d.importedId } })
                : null;
              const finalId =
