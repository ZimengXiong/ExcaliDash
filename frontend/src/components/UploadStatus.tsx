...
export const UploadStatus: React.FC = () => {
  const { tasks, clearCompleted, clearSuccessful, removeTask, isUploading } =
    useUpload();
  ...
  return (
    ...
    >
      {isOpen && (
        <div
          className="w-80 bg-white dark:bg-neutral-900 rounded-xl border-2 border-slate-800 dark:border-neutral-700 shadow-[3px_3px_0px_0px_rgba(30,41,59,0.9)] dark:shadow-[3px_3px_0px_0px_rgba(255,255,255,0.18)] overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-200 mb-2"
          role="status"
          aria-live="polite"
        >
          ...
          <div className="max-h-60 overflow-y-auto no-scrollbar p-1">
            {tasks.map((task) => (
              <div
                key={task.id}
                className="group flex items-center gap-3 p-2 hover:bg-slate-50 dark:hover:bg-neutral-800 rounded-lg transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {task.status === "error" ? (
                      <span
                        className="text-[10px] text-rose-500 font-medium truncate max-w-[80px]"
                        title={task.error}
                      >
                        {task.error || "Failed"}
                      </span>
      ...
      <button
        ...
        className={clsx(
          ...
          isOpen &&
            "bg-slate-100 dark:bg-neutral-700 translate-y-0 shadow-[1.5px_1.5px_0px_0px_rgba(30,41,59,0.9)] dark:shadow-[1.5px_1.5px_0px_0px_rgba(255,255,255,0.18)]",
        )}
      >
        {isUploading ? (
